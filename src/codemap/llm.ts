 ...
 *
 * Explicit: SECONDPAIR_PROVIDER=anthropic | openai | openrouter | openai-compatible.
 ...
 * SECONDPAIR_BASE_URL + SECONDPAIR_API_KEY.
 * PR_REVIEW_* spellings of every SECONDPAIR_* var are accepted as aliases.
 */
export function resolveProvider(): ProviderSettings {
  const env = process.env;
  const explicit = (env.SECONDPAIR_PROVIDER || env.PR_REVIEW_PROVIDER || "").toLowerCase();
  const model = env.SECONDPAIR_MODEL || env.PR_REVIEW_MODEL || env.ANTHROPIC_MODEL || "";

  const openAiCompatible = (baseUrl: string, apiKey: string): ProviderSettings => {
    if (!model) {
      throw new Error(
        "Set SECONDPAIR_MODEL (or PR_REVIEW_MODEL) when using an OpenAI-compatible provider (e.g. gpt-4o, or anthropic/claude-sonnet-4.5 on OpenRouter).",
      );
    }
    if (!apiKey) {
      throw new Error(
        "No API key found for the OpenAI-compatible provider. Set SECONDPAIR_API_KEY or PR_REVIEW_API_KEY (or OPENROUTER_API_KEY / OPENAI_API_KEY).",
      );
    }
  ...
  const customBase = env.SECONDPAIR_BASE_URL || env.PR_REVIEW_BASE_URL || env.OPENAI_BASE_URL || "";
  const genericKey = env.SECONDPAIR_API_KEY || env.PR_REVIEW_API_KEY || "";
  const cliCommand = env.SECONDPAIR_CLI_COMMAND || env.PR_REVIEW_CLI_COMMAND || "";

  const cli = (): ProviderSettings => {
    if (!cliCommand) {
      throw new Error(
        'provider "cli" needs SECONDPAIR_CLI_COMMAND (or PR_REVIEW_CLI_COMMAND), e.g. "cursor-agent -p" or "claude -p".',
      );
    }
  ...
  switch (explicit) {
    ...
    default:
      throw new Error(
        `Unknown provider "${env.SECONDPAIR_PROVIDER || env.PR_REVIEW_PROVIDER}". Use anthropic, openai, openrouter, openai-compatible, or cli.`,
      );
  }
