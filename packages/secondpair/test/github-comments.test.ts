...
describe("postReview", () => {
  ...
  it("edits the last agent review in place instead of posting a new one when there's nothing new", async () => {
    const finding: Finding = withFindingId({
      file: "src/a.ts",
      start_line: 1,
      end_line: 1,
    ...
    const commentBody = formatCommentBody(finding);
    const reviewBody = formatReviewBody(result, false, "sha1");
    ...
    const octokit = {
      paginate: vi
        .fn()
        // listPostedFindingIds -> listReviewComments
        .mockResolvedValueOnce([{ body: commentBody }])
        // findLastAgentReviewId -> listReviews
        .mockResolvedValueOnce([{ id: 99, body: reviewBody }]),
      pulls: { listReviewComments: vi.fn(), listReviews: vi.fn(), createReview, updateReview },
    } as unknown as Octokit;
    ...
    expect(updateReview).toHaveBeenCalledWith(
      expect.objectContaining({ review_id: 99 }),
    );
    expect(createReview).not.toHaveBeenCalled();
    expect(log).toHaveBeenCalledWith(expect.stringMatching(/updated the existing review/));
  });
});
