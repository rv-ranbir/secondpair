...
import {
  ...
  OPENROUTER_BASE_URL,
  resolveProvider,
} from "../../src/codemap/llm.js";

const PROVIDER_ENV_VARS = [
  "SECONDPAIR_PROVIDER",
  "SECONDPAIR_CLI_COMMAND",
...
describe("resolveProvider", () => {
  ...
  it("accepts SECONDPAIR_* env vars, taking precedence over PR_REVIEW_*", () => {
    withEnv({
      OPENROUTER_API_KEY: "sk-or-test",
      SECONDPAIR_MODEL: "secondpair-model",
      PR_REVIEW_MODEL: "legacy-model",
    });
    expect(resolveProvider().model).toBe("secondpair-model");
  });
  ...
  it("requires a model for OpenAI-compatible providers", () => {
    ...
    expect(() => resolveProvider()).toThrow(/SECONDPAIR_MODEL/);
  });
  ...
  it("routes to the cli provider when SECONDPAIR_CLI_COMMAND is set", () => {
    withEnv({ SECONDPAIR_CLI_COMMAND: "cursor-agent -p" });
    const s = resolveProvider();
  ...
  it("explicit provider=cli without a command throws a helpful error", () => {
    withEnv({ SECONDPAIR_PROVIDER: "cli" });
    expect(() => resolveProvider()).toThrow(/SECONDPAIR_CLI_COMMAND/);
  });

  it("API keys win over a cli command during auto-detection", () => {
    withEnv({ ANTHROPIC_API_KEY: "a", SECONDPAIR_CLI_COMMAND: "claude -p" });
    expect(resolveProvider().provider).toBe("anthropic");
  });
});
